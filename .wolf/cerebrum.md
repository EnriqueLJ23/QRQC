...
- [2026-07-20] CORRECCIÓN: `env_file: - stack.env` (supuesto mecanismo de Portainer) tampoco funcionó en este entorno — el usuario confirmó que seguía fallando. Solución final: NO usar env_file en absoluto; referenciar cada var directo en `environment:` del compose con sustitución `${VAR}` (Portainer sí inyecta las vars del stack como entorno del proceso `docker compose`, sin depender de ningún archivo físico). Ver docker-compose.yml (bug-017/019).
